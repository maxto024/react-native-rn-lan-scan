...
import com.rnlanscan.utils.NetworkDevice;
import com.rnlanscan.utils.NetworkScanner;
...
public class RNRnLanScanModule extends ReactContextBaseJavaModule {
    private static final String TAG = "RNLANSCANMODULE";
    ...
    public void scan(final Callback onScanFinished, final Callback onError) {
        ...
        networkScanner.scan(getReactApplicationContext(), new NetworkScanner.OnScanEventListener() {
            ...
            public void onDeviceFound(NetworkDevice device) {
                ...
                System.out.println("Devices"+device.getMac()+ device.getIP()+ device.getHostname());

                WritableNativeMap networkDeviceInfo = deviceToWriteableMap(device);

                eventEmitter.emit("EventReminder", networkDeviceInfo);
            }
